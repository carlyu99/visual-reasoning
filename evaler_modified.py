    ...
    def eval_run(self):
        ...
        try:
            for s in xrange(max_steps):
                ...
                def draw(id, img, q, target_a, pred_a):
                    if np.argmax(target_a) == np.argmax(pred_a) and np.random.randint(0, 20) == 0:
                        plt.imshow(img)
                        plt.title(question2str(q))
                        plt.xlabel(answer2str(target_a) + answer2str(pred_a, 'Predicted'))

                        folder = 'visualization/pred_correct'
                        if not os.path.isdir(folder):
                            os.mkdir(folder)
                        plt.savefig(os.path.join(folder, id.decode('utf8')))
                        # plt.show()

                for i in range(self.batch_size):
                    draw(batch_chunk['id'][i], batch_chunk['img'][i],
                         batch_chunk['q'][i], batch_chunk['a'][i], prediction_pred[i])
