...
class Model(object):
    ...
    def build(self, is_train=True):
        ...
        feature16 = tf.stack([conv_single(self.img[:, i]) for i in range(16)], axis=1)
        self.feature16 = feature16
        print(self.feature16.shape)
        feature16 = [feature16[:, :, cut[i]: cut[i + 1]] for i in range(len(cut) - 1)]
        score = []
        for choice in tqdm(range(8, 16)):
            sk = []
            for i in range(len(cut) - 1):
                feature9 = [feature16[i][:, j] for j in list(range(8)) + [choice]]
